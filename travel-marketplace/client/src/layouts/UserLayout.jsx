        ...
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-bold text-white ring-2 ring-white animate-badge-pulse">
              ...
              {profileOpen && (
                <>
                  ...
                  <div className="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-xl border border-slate-100 bg-white py-1 shadow-glass animate-scale-in">
                    ...
                    <NavLink to="/app/referrals" onClick={() => setProfileOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 transition hover:bg-slate-50">
                      <svg className="h-4 w-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" /></svg>
                      Refer & Earn
                    </NavLink>
                    <NavLink to="/app/analytics" onClick={() => setProfileOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 transition hover:bg-slate-50">
          ...
          <div className="border-t border-slate-100 bg-white/95 backdrop-blur-lg px-4 py-3 md:hidden animate-slide-down">
            <div className="flex flex-col gap-1">
              ...
              <NavLink to="/app/cart" onClick={() => setMobileOpen(false)} className={linkClass}>
                Cart {cartCount > 0 && <span className="ml-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-bold text-white">{cartCount}</span>}
              </NavLink>
              <NavLink to="/app/profile" onClick={() => setMobileOpen(false)} className={linkClass}>
                Profile
              </NavLink>
              <NavLink to="/app/referrals" onClick={() => setMobileOpen(false)} className={linkClass}>
                🎁 Refer & Earn
              </NavLink>
              <NavLink to="/app/analytics" onClick={() => setMobileOpen(false)} className={linkClass}>
                📊 My Analytics
              </NavLink>
